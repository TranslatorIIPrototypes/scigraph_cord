...
class Normalizer():
    ...
    def _fix_type_labels(self):
        prefix_to_types = {'MONDO':['disease', 'named_thing', 'biological_entity', 'disease_or_phenotypic_feature'],
        'EFO': ['phenotypic_feature', 'named_thing', 'biological_entity', 'disease_or_phenotypic_feature'],
        'ECTO': ['chemical_exposure', 'exposure_event', 'named_thing'],
        'ENVO': ['environmental_feature', 'planetary_entity', 'named_thing'],
        'HANCESTRO': ['population_of_individual_organisms', 'named_thing'],
        'FAO':['anatomical_entity', 'organismal_entity', 'biological_entity', 'named_thing'],
        'PO':['anatomical_entity', 'organismal_entity', 'biological_entity', 'named_thing'],
        'CHEBI':['chemical_substance', 'molecular_entity', 'biological_entity', 'named_thing'],
        'HsapDv': ['life_stage','organismal_entity','named_thing'],
        'FOODON':['food', 'named_thing'] }
        for gc,types in self.curie_to_type.items():
            if len(types) == 1:
                if types[0] != 'named_thing':
                    print('seems wrong')
                    exit()
